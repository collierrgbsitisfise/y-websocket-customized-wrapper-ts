  ...
  try {
    ...
    switch (messageType) {
      ...
      case messageUserList:
        // This case is handled server-side, so we don't need to do anything here
        break
    }
  } catch (err) {
    console.error(err)
...
class WSSharedDoc extends Y.Doc {
  ...
  notifyClientsAboutUsers() {
    const encoder = encoding.createEncoder()
    encoding.writeVarUint(encoder, messageUserList)
    encoding.writeVarString(encoder, JSON.stringify(Array.from(this.connectedClients.values())))
    const buff = encoding.toUint8Array(encoder)
    this.conns.forEach((_, c) => {
      send(this, c, buff)
    })
 ...
 * @param {WSSharedDoc} doc
 * @param {any} conn
 * @param {string} uniqueClientIdentifier
 */
const closeConn = (doc, conn, uniqueClientIdentifier) => {
  if (doc.conns.has(conn)) {
    ...
    doc.connectedClients.delete(uniqueClientIdentifier)
...
exports.setupWSConnection = (conn, req, { docName = req.url.slice(1).split('?')[0], gc = true, uniqueClientIdentifier = '', name = '' } = {}) => {
  ...
  const pingInterval = setInterval(() => {
    if (!pongReceived) {
      if (doc.conns.has(conn)) {
        closeConn(doc, conn, uniqueClientIdentifier)
      }
  ...
  // Send the initial user list to the new client
  {
    const encoder = encoding.createEncoder()
    encoding.writeVarUint(encoder, messageUserList)
    encoding.writeVarString(encoder, JSON.stringify(Array.from(doc.connectedClients.values())))
    send(doc, conn, encoding.toUint8Array(encoder))
  }
}
